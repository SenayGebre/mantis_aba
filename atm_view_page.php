...
<?php
		foreach( $t_atms_related as $t_atm ) {
			...
			<div class="col-md-3 col-xs-6 no-padding"><a href="atm_view_page.php?atm_id=<?php echo $t_atm['id']; ?>" title="<?php echo $t_description; ?>"><?php echo $t_name; ?></a></div>
...
<?php
		if( $t_can_edit_own ) {
?>
		<form class="form-inline pull-left" action="atm_update_page.php" method="post">
			<fieldset>
		...
		if( $t_can_edit ) { ?>
		<form class="form-inline pull-left" action="atm_delete.php" method="post">
			<fieldset>
				<?php echo form_security_field( 'atm_delete' ) ?>
				<input type="hidden" name="atm_id" value="<?php echo $f_atm_id ?>" />
