...
function mc_atm_add( $p_username, $p_password, stdClass $p_atm ) {
	...
	$t_atm_name = $p_atm['name'];
	$t_atm_description = array_key_exists( 'description', $p_atm ) ? $p_atm['description'] : '';

	if( !atm_name_is_valid( $t_atm_name, $t_valid_matches ) ) {
		return ApiObjectFactory::faultBadRequest( 'Invalid atm name : "' . $t_atm_name . '"' );
	}

	$t_matching_by_name = atm_get_by_name( $t_atm_name );
	if( $t_matching_by_name != false ) {
		return ApiObjectFactory::faultConflict( 'A atm with the same name already exists , id: ' . $t_matching_by_name['id'] );
	}

	log_event( LOG_WEBSERVICE, 'creating atm \'' . $t_atm_name . '\' for user \'' . $t_user_id . '\'' );
	return atm_create( $t_atm_name, $t_user_id, $t_atm_description );
}
...
function mci_atm_set_for_issue ( $p_issue_id, array $p_atms, $p_user_id ) {
	...
	foreach( $p_atms as $t_atm ) {
		...
		if( isset( $t_atm['id'] ) ) {
			...
			$t_get_atm = atm_get_by_name( $t_atm['name'] );
			if( $t_get_atm === false ) {
				throw new ClientException(
					"ATM '{$t_atm['name']}' not found.",
					ERROR_ATM_NOT_FOUND
				);
			}
