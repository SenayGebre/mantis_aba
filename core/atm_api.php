...
function atm_cache_rows( array $p_atm_ids ) {
	...
	$t_query = 'SELECT * FROM {atm} where id IN (' . implode( ',', $t_sql_in_params ) . ')';
...
function atm_cache_bug_atm_rows( array $p_bug_ids ) {
	...
	$t_query = 'SELECT B.id AS bug_id, BT.atm_id, BT.user_id, BT.date_attached FROM {bug} B LEFT OUTER JOIN {bug_atm} BT ON B.id=BT.bug_id'
 ...
 * Uses a case-insensitive search of the database for existing atms with the same name.
 * @param string $p_name The atm name to check.
 * @return boolean True if name is unique
 */
function atm_is_unique( $p_name ) {
	$c_name = trim( $p_name );

	$t_query = 'SELECT id FROM {atm} WHERE ' . db_helper_like( 'name' );
 ...
 * @return void
 */
function atm_ensure_unique( $p_name ) {
	if( !atm_is_unique( $p_name ) ) {
 ...
 * which by default only includes the valid atm name itself.
 ...
 */
function atm_name_is_valid( $p_name, array &$p_matches, $p_prefix = '' ) {
...
/**
 * Ensure a atm name is valid.
 * @param string $p_name The atm name to check.
 * @return void
 */
function atm_ensure_name_is_valid( $p_name ) {
	$t_matches = array();
	if( !atm_name_is_valid( $p_name, $t_matches ) ) {
		error_parameters( $p_name );
...
/**
 * Compare two atm rows based on atm name.
 ...
 */
function atm_cmp_name( array $p_atm1, array $p_atm2 ) {
	return strcasecmp( $p_atm1['name'], $p_atm2['name'] );
}

/**
 * Parse a form input string to extract existing and new atms.
 * When given a string, parses for atm names separated by configured separator,
 ...
 * id = -1 and the atm name, and if the name is invalid, a row is returned with
 * id = -2 and the atm name.  The resulting array is then sorted by atm name.
...
function atm_parse_string( $p_string ) {
	...
	foreach( $t_strings as $t_name ) {
		...
		$t_matches = array();
		$t_atm_row = atm_get_by_name( $t_name );
		if( $t_atm_row !== false ) {
			$t_atms[] = $t_atm_row;
		} else {
			if( atm_name_is_valid( $t_name, $t_matches ) ) {
				$t_id = -1;
			} else {
				$t_id = -2;
			}
...
function atm_attach_many( $p_bug_id, $p_atm_string, $p_atm_id = 0 ) {
	...
	foreach( $t_atms_create as $t_atm_row ) {
		$t_atm_row['id'] = atm_create( $t_atm_row['name'] );
		$t_atms_attach[] = $t_atm_row;
	}
...
function atm_parse_filters( $p_string ) {
	...
	foreach( $t_strings as $t_name ) {
		...
		if( !is_blank( $t_name ) && atm_name_is_valid( $t_name, $t_matches, $t_prefix ) ) {
			$t_atm_row = atm_get_by_name( $t_matches[1] );
			if( $t_atm_row !== false ) {
 ...
 *
 * @param integer $p_name_filter A string to match the beginning of the atm name.
 * @param integer $p_count       The number of atms to return.
 * @param integer $p_offset      The offset of the result.
 *
 * @return ADORecordSet|boolean ATMs sorted by name, or false if the query failed.
 */
function atm_get_all( $p_name_filter, $p_count, $p_offset ) {
	$t_where = '';
	...
	$t_query = 'SELECT * FROM {atm} ' . $t_where . ' ORDER BY name';
 ...
 * @return integer
 */
function atm_count( $p_name_filter ) {
	$t_where = '';
	$t_where_params = array();
	...
	$t_query = 'SELECT count(*) FROM {atm}' . $t_where;
 ...
 * Get atm name by id.
 * @param integer $p_atm_id The atm ID to retrieve from the database.
 * @return string atm name or empty string if not found.
 */
function atm_get_name( $p_atm_id ) {
	$t_atm_row = atm_get( $p_atm_id );
	if( $t_atm_row === false ) {
		return '';
	}

	return $t_atm_row['name'];
}

/**
 * Return a atm row for the given name.
 * @param string $p_name The atm name to retrieve from the database.
 * @return array|false ATM row
 */
function atm_get_by_name( $p_name ) {
	db_param_push();
	$t_query = 'SELECT * FROM {atm} WHERE ' . db_helper_like( 'name' );
...
/**
 * Create a atm with the given name, creator, and description.
 * Defaults to the currently logged in user, and a blank description.
 * @param string  $p_name        The atm name to create.
 * @param integer $p_user_id     The user ID to link the new atm to.
 * @param string  $p_description A Description for the atm.
 * @return int ATM ID
 */
function atm_create( $p_name, $p_user_id = null, $p_description = '' ) {
	atm_ensure_can_create( $p_user_id );

	atm_ensure_name_is_valid( $p_name );
	atm_ensure_unique( $p_name );
...
/**
 * Update a atm with given name, creator, and description.
 * @param integer $p_atm_id      The atm ID which is being updated.
 * @param string  $p_name        The name of the atm.
 ...
 */
function atm_update( $p_atm_id, $p_name, $p_user_id, $p_description ) {
	$t_atm_row = atm_get( $p_atm_id );
	$t_atm_name = $t_atm_row['name'];

	if( $t_atm_name == $p_name &&
		 $t_atm_row['description'] == $p_description &&
	...
	$t_rename = false;
	if( mb_strtolower( $p_name ) != mb_strtolower( $t_atm_name ) ) {
		atm_ensure_unique( $p_name );
		$t_rename = true;
	}
	...
	$t_query = 'UPDATE {atm}
					...
					WHERE id=' . db_param();
	db_query( $t_query, array( (int)$p_user_id, $p_name, $p_description, $c_date_updated, $p_atm_id ) );

	if( $t_rename ) {
		$t_bugs = atm_get_bugs_attached( $p_atm_id );

		foreach( $t_bugs as $t_bug_id ) {
			history_log_event_special( $t_bug_id, ATM_RENAMED, $t_atm_name, $p_name );
		}
	}
...
function atm_delete( $p_atm_id ) {
	...
	db_param_push();
	$t_query = 'DELETE FROM {atm} WHERE id=' . db_param();
 ...
 *
 * @return array List of atm rows, each with id, name, and description.
 */
function atm_get_candidates_for_bug( $p_bug_id ) {
	db_param_push();
	$t_query = 'SELECT id, name, description FROM {atm}';
	$t_params = array();

	if( 0 != $p_bug_id ) {
		$t_assoc_atms_query = 'SELECT atm_id FROM {bug_atm} WHERE bug_id = ' . db_param();
		$t_params[] = $p_bug_id;
...
function atm_bug_is_attached( $p_atm_id, $p_bug_id ) {
	db_param_push();
	$t_query = 'SELECT bug_id FROM {bug_atm} WHERE atm_id=' . db_param() . ' AND bug_id=' . db_param();
...
function atm_bug_get_row( $p_atm_id, $p_bug_id ) {
	...
	if( !$t_bug_atms || !isset( $t_bug_atms[$p_atm_id] ) ) {
		trigger_error( ATM_NOT_ATTACHED, ERROR );
	}
	return $t_bug_atms[$p_atm_id];
}

/**
 * Return an array of atms attached to a given bug sorted by atm name.
 * @param integer $p_bug_id The bug ID to check.
...
function atm_bug_get_attached( $p_bug_id ) {
	...
	usort( $t_atm_info_rows, 'atm_cmp_name' );
...
function atm_get_bugs_attached( $p_atm_id ) {
	db_param_push();
	$t_query = 'SELECT bug_id FROM {bug_atm} WHERE atm_id=' . db_param();
...
function atm_bug_attach( $p_atm_id, $p_bug_id, $p_user_id = null ) {
	...
	if( atm_bug_is_attached( $p_atm_id, $p_bug_id ) ) {
		trigger_error( ATM_ALREADY_ATTACHED, ERROR );
	}
	...
	$t_atm_name = atm_get_field( $p_atm_id, 'name' );
	history_log_event_special( $p_bug_id, ATM_ATTACHED, $t_atm_name );
...
function atm_bug_detach( $p_atm_id, $p_bug_id, $p_add_history = true, $p_user_id = null ) {
	...
	if( !access_has_bug_level( $t_detach_level, $p_bug_id, $t_user_id ) ) {
		throw new ClientException(
			sprintf( "Access denied to detach '%s'", $t_atm_row['name'] ),
			ERROR_ACCESS_DENIED
		);
	}

	db_param_push();
	$t_query = 'DELETE FROM {bug_atm} WHERE atm_id=' . db_param() . ' AND bug_id=' . db_param();
	...
	if( $p_add_history ) {
		$t_atm_name = atm_get_field( $p_atm_id, 'name' );
		history_log_event_special( $p_bug_id, ATM_DETACHED, $t_atm_name );
	}
...
function atm_get_link( array $p_atm_row ) {
	return sprintf(
		...
		string_display_line( $p_atm_row['name'] )
	);
}
...
function atm_display_link( array $p_atm_row, $p_bug_id = 0 ) {
	...
	if( isset( $p_atm_row['user_attached'] ) && auth_get_current_user_id() == $p_atm_row['user_attached']
	...
	if( $p_bug_id > 0 && access_has_bug_level( $t_detach, $p_bug_id ) ) {
		$t_tooltip = string_html_specialchars( sprintf( lang_get( 'atm_detach' ), string_display_line( $p_atm_row['name'] ) ) );
...
function atm_stats_attached( $p_atm_id ) {
	db_param_push();
	$t_query = 'SELECT COUNT(*) FROM {bug_atm} WHERE atm_id=' . db_param();
	$t_result = db_query( $t_query, array( $p_atm_id ) );
...
function atm_stats_related( $p_atm_id, $p_limit = 5 ) {
	...
	$t_sql = 'SELECT atm_id, COUNT(1) AS atm_count FROM {bug_atm}'
